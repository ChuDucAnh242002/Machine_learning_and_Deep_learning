...
for c in classes:
    ...
    sigma2 = np.var(x_c, axis=0)
    stats[c] = [mu, sigma2]


def cal_gauss(x, stats):
    # Calculate probability and return the class
    bayes_ps = np.empty([x.shape[0], len(stats.keys())])
    log_pi = np.log(2*np.pi)
    for c in stats.keys():
        mu = stats[c][0]
        sigma2 = stats[c][1]
        bayes_ps[:,c] = np.sum(-1/2 * (log_pi + np.log(sigma2) + ((x - mu) ** 2) / sigma2), axis=1)
    return np.argmax(bayes_ps, axis=1)


y_pred = cal_gauss(x_test_vector, stats)
